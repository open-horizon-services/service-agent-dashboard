# Service Agent Dashboard - Docker container monitoring dashboard for edge IoT devices

DOCKER_HUB_ID ?= tfine
MATCH ?= "Service Agent Dashboard"
TIME_OUT ?= 30

# The Open Horizon Exchange's organization ID namespace where you will be publishing files
export HZN_ORG_ID ?= myorg

export SERVICE_NAME ?= service-agent-dashboard
PATTERN_NAME ?= pattern-service-agent-dashboard
DEPLOYMENT_POLICY_NAME ?= deployment-policy-service-agent-dashboard
NODE_POLICY_NAME ?= node-policy-service-agent-dashboard
...
test:
	...
	@echo "=================="
	@curl -sS http://127.0.0.1:5000/api/system
